...
class LSTMEncoder(nn.Module):
    def __init__(self, input_dim, latent_dim, condition_dim, hidden_dim=128, num_layers=1, dropout_prob=0.2):
        super(LSTMEncoder, self).__init__()
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers=num_layers, batch_first=True, dropout=dropout_prob)
        self.fc_mu = nn.Linear(hidden_dim + condition_dim, latent_dim)  # Latent mean
        self.fc_logvar = nn.Linear(hidden_dim + condition_dim, latent_dim)  # Latent log variance
    ...
    def forward(self, x, c):
        ...
        # h_n의 마지막 layer의 hidden state만 사용
        h_n = h_n[-1]  # [batch_size, hidden_dim]

        # Condition vector와 결합
        h = torch.cat([h_n, c], dim=-1)  # [batch_size, hidden_dim + condition_dim]

        mu = self.fc_mu(h)  # Latent mean
...
class LSTMDecoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim, output_dim, condition_dim, num_layers=1, dropout_prob=0.2):
        super(LSTMDecoder, self).__init__()
        self.fc = nn.Linear(latent_dim + condition_dim, hidden_dim)  # Latent vector와 condition을 입력으로 받음
        self.lstm = nn.LSTM(hidden_dim, hidden_dim, num_layers=num_layers, batch_first=True, dropout=dropout_prob)
        self.fc_output = nn.Linear(hidden_dim, output_dim)  # 최종 출력

    def forward(self, z, c):
        # z: Latent vector [batch_size, latent_dim]
        # c: condition vector [batch_size, condition_dim]

        # Latent vector와 condition vector 결합
        z = torch.cat([z, c], dim=-1)  # [batch_size, latent_dim + condition_dim]

        # FC를 통해 차원 변환
        h = F.gelu(self.fc(z)).unsqueeze(1)  # [batch_size, 1, hidden_dim]

        # LSTM을 통해 시계열 형태로 디코딩
        h, _ = self.lstm(h)  # [batch_size, time_steps, hidden_dim]

        # 시계열 출력에 대해 최종 출력 계산
        x_recon = torch.sigmoid(self.fc_output(h.squeeze(1)))  # [batch_size, output_dim]
...
class LSTMCVAE(nn.Module):
    def __init__(self, input_dim, latent_dim, hidden_dim, condition_dim, dropout_prob=0.2):
        ...
        self.encoder = LSTMEncoder(input_dim, latent_dim, condition_dim, hidden_dim, dropout_prob=dropout_prob)
        self.decoder = LSTMDecoder(latent_dim, hidden_dim, input_dim, condition_dim, dropout_prob=dropout_prob)
