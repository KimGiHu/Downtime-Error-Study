...
class Encoder(nn.Module):
    def __init__(self, input_dim, latent_dim, condition_dim, dropout_prob=0.2):
        ...
        self.fc_mu = nn.Linear(512 + condition_dim, latent_dim)  # Latent mean
        self.fc_logvar = nn.Linear(512 + condition_dim, latent_dim)  # Latent log variance
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim, output_dim, condition_dim, dropout_prob=0.2):
        ...
        self.fc2 = nn.Linear(512, 512)
        self.fc3 = nn.Linear(512, 512 * hidden_dim)
        self.fc_output = nn.Linear(512, output_dim)
        self.dropout = nn.Dropout(dropout_prob)
        self.bn1 = nn.BatchNorm1d(512 * hidden_dim)
    ...
    def forward(self, z, c):
        ...
        h = F.gelu(self.fc3(h))
        h = self.bn1(h)
        h = h.view(h.size(0), self.hidden_dim, 512)  # [batch_size, 1, 128]
...
# 조건부-번이 오토인코더 모델 정의
model = CVAE(input_dim, latent_dim, hidden_dim, condition_dim, dropout).to(device)
