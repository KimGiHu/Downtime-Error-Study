...
parser.add_argument('--fig', type = str,
                    default = './figure/0910/CVAE', help='Enter the stored figure path')
parser.add_argument('--fl', type = int,
                    default = 13, help='Enter which file list it is')
parser.add_argument('--case', type = int,
                    default = 13, help='Enter which case it is')
parser.add_argument('--data_err', type = int,
                    default = 13, help='Enter which device it is')
parser.add_argument('--start_date', type = str,
                    default = '2023-09-01 00:00:00', help='Enter the date where it is started')
parser.add_argument('--end_date', type = str,
                    default = '2023-09-14 23:59:59', help='Enter the date where it is ended')
parser.add_argument('--threshold', type = float,
                    default = 0.06, help='Enter the value of threshold')
parser.add_argument('--number', type = int,
                    default = 1, help='Enter data number before error')
parser.add_argument('--number_abnormal', type = int,
                    default = 2, help='Enter data number after error')
parser.add_argument('--del_front', type = int,
                    default = 0, help='Enter data number which is eliminated')
parser.add_argument('--del_end', type = int,
                    default = 10, help='Enter data number which is abnormal')                   
...
def plot(df, name, ylimit=None):
    ...
    createDirectory('%s'%args.fig)
    plt.savefig('%s'%args.fig+'/%s.png'%name, dpi=600)
    plt.clf() # figure 초기화
...
# 날짜 범위 설정
start_date = '%s'%args.start_date
end_date = '%s'%args.end_date

filtered_df = df.loc[args.start_date:args.end_date]

plot(filtered_df, '2 weeks filtered data')

# 이상이 있는 데이터의 넘버 입력 및 그래프 보여주기
data_err = args.data_err
plot(filtered_df[f"R:QMP{data_err}"], f'filter data R:QMP-{data_err} plot')
...
number= args.number
# 이상 데이터 넘버 입력 
number_abnormal = args.number_abnormal

# 제거할 값 입력, 이상 데이터는 그대로 출력
del_front = args.del_front
del_end = args.del_end
k = 1
for i in [number]:
    fix_df = dfs[f'df{i}']
...
# reshape inputs for Auto-Encoder [samples, timesteps, features]
...
# 조건부-번이 오토인코더 모델 정의
model = CVAE_MLP(input_dim, latent_dim, hidden_dim, condition_dim, dropout).to(device)
...
torch.save(model.state_dict(), "%s"%args.path+f"case_{args.case}.pth")
...
plt.savefig('%s'%args.fig+'/Model loss.png', dpi=600)
...
with torch.no_grad():
    ...
    # 플롯 저장
    plt.savefig('%s'%args.fig+'/Loss Distribution.png', dpi=600)
    plt.close()
...
with torch.no_grad():
    ...
    Xtest = X_test.cpu().numpy().reshape(X_test.shape[0], X_test.shape[-1])
...
ax.set_xlabel('Time')
# 두 번째 플롯 표시
plt.savefig('%s'%args.fig+'/Loss graph.png', dpi=600)
...
ax.set_ylabel('Loss (mse)')
ax.set_xlabel('Time')
plt.savefig('%s'%args.fig+'/New Score Based Loss graph.png', dpi=600)
scored_reset.to_csv(f'./score/Case{args.case+1}.csv', index=True)
